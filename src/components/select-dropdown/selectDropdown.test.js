...
describe('SelectDropdown', () => {
  ...
  it('should not have empty list', () => {
    ...
    const listElement = screen.getByRole('list');
    const listItems = within(listElement).queryAllByRole('listitem');
    expect(listItems).toHaveLength(privacyList.length);
  });
  ...
  it('should have list items', () => {
    ...
    const icons = within(listElement).getByRole('img');
  ...
  it('should display selected item', () => {
    ...
    const selectedItem = screen.getByTestId('time-text-display');
    const selectedItemText = screen.getByTestId('selected-item-text');
    expect(selectedItem).toBeInTheDocument();
    expect(selectedItemText).toBeInTheDocument();
    expect(selectedItemText).toHaveTextContent('Public');
  });
});
